...
int main(int argc, char **argv) {
    ...
    for (long int i = 0; i < h * w * c; i++) cin >> h_mat[i];
    ...
    for (long int i = 0; i < r * s * c * k; i++) cin >> h_filter[i];
    ...
    cudaMalloc(&d_result, sizeof(long int) * h * w * k);

    // Copy data from host to device
    cudaMemcpy(d_matrix, h_mat, sizeof(long int) * h * w * c, cudaMemcpyHostToDevice);
    ...
    // Shared memory size: input matrix patch size + filter size
    size_t shared_mem_size = ((blockDim.x + s - 1) * (blockDim.y + r - 1) + r * s) * sizeof(long int);

    // Launch kernel
    dkernel<<<gridDim, blockDim, shared_mem_size>>>(d_matrix, d_filter, d_result, h, w, c, r, s, k);
    ...
    std::chrono::duration elapsed1 = end - start;
