...
__global__ void CalculateInvertedGrayScale(long int *red, long int *green, long int *blue, 
                                         long int *T1, int m, int n) {
    //calculate the linear index                                                                                  
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    
    if (idx < m * n) {
                
        long int gray = floor((red[idx] + green[idx] + blue[idx]) / 3.0);
        
        //calculate matrix index from linear
        int i = idx / n;
        int j = idx % n;

        //calculate inverted linear index from matrix
        int temp = ((m-1-i) * n) + j;
        T1[temp] = gray;
    }
}
__global__ void CalculateThomasTransformation(long int *red, long int *green, long int *blue, 
                                            long int *T2, int m, int n) {
    //calculate the linear index                    
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    
    if (idx < m * n) {
        // Calculate transformed value
        long int new_red = floor((0.5 * red[idx]));
        long int new_green = floor(sqrtf(green[idx]));
        T2[idx] = new_red + new_green + blue[idx];
    }
}
