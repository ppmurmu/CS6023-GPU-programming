...
private:
    ...
    // Find the closest shelter from a given city with remaining capacity
    int findClosestShelter(int city, bool forElderly = false)
    {
        ...
        for (int shelter : shelterCities)
        {
            ...
            {
                int dist = distances[city][shelter];

                // Check if distance is valid for elderly
                if (forElderly && dist > max_distance_elderly)
                    continue;

                if (dist < minDistance && dist != INT_MAX)
                {
                    minDistance = dist;
...
public:
        ...
        int num_populated_cities)
    {
        ...
        for (int i = 0; i < num_populated_cities; i++)
        {
            ...
            {
                // Find closest shelter for elderly if there are any
                int nextShelter = -1;
                if (elderly > 0)
                {
                    nextShelter = findClosestShelter(currentCity, true);
                }

                // If no shelter for elderly or no elderly left, find for prime age
                if (nextShelter == -1 && prime_age > 0)
                {
                    nextShelter = findClosestShelter(currentCity, false);
                }

                // If no shelter available, drop everyone at current city
                if (nextShelter == -1)
                {
                    if (elderly > 0 || prime_age > 0)
                    {
                        evacDrops.push_back({(long long)currentCity, (long long)prime_age, (long long)elderly});
                    }
                    break;
                }

                // Get path to the shelter
                vector<int> pathToShelter = shortestPaths[currentCity][nextShelter];
