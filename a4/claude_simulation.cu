...
// Global road availability times (in minutes)
static unordered_map<long long, double> roadNextAvailableTime;

// Helper to compute unique key for an edge
inline long long getEdgeKey(int u, int v, int num_cities)
{
    return (long long)u * num_cities + v;
}
    ...
    int num_populated_cities)
{
    ...
    for (int i = 0; i < num_populated_cities; i++)
    {
        ...
        double currentTime = 0.0; // in minutes
        ...
        // Drop at source if shelter
        if (shelterCapacity[sourceCity] > 0)
        {
            int cap = shelterCapacity[sourceCity];
            int dropE = min(elderly, cap);
            cap -= dropE;
            elderly -= dropE;
            int dropP = min(prime_age, cap);
            cap -= dropP;
            prime_age -= dropP;
            shelterCapacity[sourceCity] -= (dropE + dropP);
            if (dropE + dropP > 0)
                evacDrops.push_back({sourceCity, dropP, dropE});
        }
        ...
        while (prime_age > 0 || elderly > 0)
        {
            ...
            targetShelter = (nextShelterForElderly != -1) ? nextShelterForElderly : nextShelterForPrime;

            // Get path to chosen shelter
            ...
            // Travel along the path with contention & batching
            for (int j = startIdx; j < (int)pathToShelter.size(); j++)
            {
                int nextCity = pathToShelter[j];
                int roadLength = 0, roadCapacity = 1;
                // find road info
                for (const Road &r : adjacencyList[currentCity])
                {
                    if (r.to == nextCity)
                    {
                        roadLength = r.length;
                        roadCapacity = r.capacity;
                        break;
                    }
                }
                // check elderly distance
                bool elderlyCanReach = (distanceTraveled + roadLength) <= max_distance_elderly;
                if (elderly > 0 && !elderlyCanReach)
                {
                    evacDrops.push_back({(long long)currentCity, 0LL, (long long)elderly});
                    elderly = 0;
                }

                // Compute batching and contention
                int movingGroup = prime_age + elderly;
                int batches = (movingGroup + roadCapacity - 1) / roadCapacity;
                double travelTime = roadLength * 12.0 * batches; // minutes (12 min per km)
                long long keyUV = getEdgeKey(currentCity, nextCity, num_cities);
                double departTime = max(currentTime, roadNextAvailableTime[keyUV]);
                double arrivalTime = departTime + travelTime;
                // update road availability both ways
                roadNextAvailableTime[keyUV] = arrivalTime;
                roadNextAvailableTime[getEdgeKey(nextCity, currentCity, num_cities)] = arrivalTime;
                currentTime = arrivalTime;

                // move to next city
                evacPath.push_back(nextCity);
                currentCity = nextCity;
                distanceTraveled += roadLength;

                // Drop if shelter
                if (shelterCapacity[currentCity] > 0)
                {
                    int cap = shelterCapacity[currentCity];
                    int dropE = min(elderly, cap);
                    cap -= dropE;
                    elderly -= dropE;
                    int dropP = min(prime_age, cap);
                    cap -= dropP;
                    prime_age -= dropP;
                    shelterCapacity[currentCity] -= (dropE + dropP);
                    if (dropE + dropP > 0)
                        evacDrops.push_back({(long long)currentCity, dropP, dropE});
        ...
        for (int j = 0; j < (int)evacPath.size(); j++)
            paths[i][j] = evacPath[j];

        num_drops[i] = evacDrops.size();
        drops[i] = new long long *[evacDrops.size()];
        for (int j = 0; j < (int)evacDrops.size(); j++)
        {
            drops[i][j] = new long long[3];
            drops[i][j][0] = evacDrops[j][0];
            drops[i][j][1] = evacDrops[j][1];
            drops[i][j][2] = evacDrops[j][2];
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_roads; i++)
    {
        ...
        // Initialize road availability
        long long keyUV = getEdgeKey(u, v, num_cities);
        roadNextAvailableTime[keyUV] = 0.0;
        roadNextAvailableTime[getEdgeKey(v, u, num_cities)] = 0.0;
    }
